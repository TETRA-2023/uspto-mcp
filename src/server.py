...
@mcp.tool(
    ...
    description=(
        ...
        "Accepts plain English ('graphene battery') or explicit BRS syntax "
        "('graphene AND battery', '(\"6103599\").pn.'). Plain English queries "
        "are automatically rewritten to AND form before the search runs — "
        "'graphene battery' becomes 'graphene AND battery', so results are "
        "intersections, not unions. To query for an exact phrase, wrap in "
        "double-quotes: '\"prior art\"'. If a quoted-phrase query returns zero "
        "results, the tool automatically retries in AND form and returns both "
        "outcomes under an auto_retry key. Default sources: US-PGPUB (published "
        "applications), USPAT (granted patents), USOCR (OCR'd older patents). "
        "Returns a paginated envelope of summary-tier records (~18 fields at "
        "standard verbosity: id, title, applicant/assignee, classification, "
        "dates, family ID, score). NOTE: summary records do NOT include abstract "
        "or claims. To read a full document, follow up with "
        "`ppubs_get_patent_by_number(publication_number=<record."
        "publicationReferenceDocumentNumber>)`. Pass verbosity='full' to include "
        "all 89 raw fields per record (useful for batch analysis)."
    ),
)
...
@mcp.tool(
    ...
    description=(
        ...
        "fetching any documents. Cheaper and faster than `ppubs_search_patents` "
        "— ideal for answering 'how many?' questions or tuning a query before "
        "running the full search. Accepts plain English ('graphene battery') or "
        "explicit BRS syntax ('graphene AND battery'). Plain English queries are "
        "automatically rewritten to AND form — 'graphene battery' counts the "
        "intersection (~53k), not the union (~2.4M). If a quoted-phrase query "
        "returns zero, the tool retries in AND form and returns both counts under "
        "an auto_retry key. Default sources: US-PGPUB, USPAT, USOCR. Returns "
        "{total, query, sources}. Follow up with `ppubs_search_patents` to list "
        "matching records."
    ),
)
