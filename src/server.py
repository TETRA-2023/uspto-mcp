...
@mcp.tool(
    ...
    description=(
        ...
        "AND operands, and do NOT rewrite AND operands as phrases to narrow broad "
        "results** — when a query returns too many hits, add more AND terms "
        "('AI AND agent AND prosecution AND claim') rather than grouping existing "
        'terms into quotes (\'"AI agent" AND "patent prosecution"\' often returns '
        "zero for the same concept). Phrase quotes are only appropriate for "
        "ultra-specific terms that are meaningless word-by-word (e.g. "
        "'\"prior art\"', '\"inter partes\"'). Default sources: "
...
@mcp.tool(
    ...
    description=(
        ...
        "**Do NOT quote individual AND operands, and do NOT rewrite AND "
        "operands as phrases to narrow a high count** — add more AND terms "
        "instead ('AI AND agent AND prosecution' → too many → add 'AND claim'). "
        "Phrase quotes are only appropriate for ultra-specific terms meaningless "
        "word-by-word (e.g. '\"prior art\"'). "
